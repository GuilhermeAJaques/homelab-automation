...
*   **Edge**: a Raspberry Pi 3 acting as a multi-protocol IoT gateway, running either the Python (Project 01) or the C (Project 02) collector in this repository. Both implement the same five field protocols behind the same connection/configuration model, and either one can drive the same MQTT/REST/dashboard stack. The Pi also runs Grafana Loki natively, receiving structured application logs from whichever collector is active.
...
<table><tbody><tr><td>Project</td><td>Description</td></tr><tr><td><a href="project-00-setup/">project-00-setup</a></td><td>Docker cloud stack (Portainer, InfluxDB, Grafana, Mosquitto, Node-RED, MQTT-to-InfluxDB subscriber) and Raspberry Pi base setup, including the Grafana Loki logging service.</td></tr><tr><td><a href="project-01-multiprotocol-python/">project-01-multiprotocol-python</a></td><td>Python multi-protocol IoT gateway: connects to Siemens S7, Rockwell EtherNet/IP, Modbus TCP, OPC-UA, and GPIO, and publishes to MQTT, a REST API, and Grafana Loki.</td></tr><tr><td><a href="project-02-multiprotocol-c/">project-02-multiprotocol-c</a></td><td>The same gateway reimplemented from scratch in C, working directly against each protocol's native library instead of a high-level automation framework, with the same MQTT, REST API, and logging surface.</td></tr></tbody></table>
