...
A new folder on /python/connections/ must be created with name Connection_N (N is the number of driver), with two files inside driver.txt and variables.csv, inside the folder already has some examples how to configure each driver
...
* GET:
  ...
  * http://IP:5000/variable/variable_topic

    * ip must be replaced by IoT device IP
    * variable_topic: must be replaced by topic configured at connections
...
* POST:
  ...
  * http://ip:5000/write
    ...
    * Body must be configured as json with:
      ...
      * {"topic": "variable_topic", "value": value}
      * ip must be replaced by IoT device IP
      * variable_topic: must be replaced by topic configured at connections
